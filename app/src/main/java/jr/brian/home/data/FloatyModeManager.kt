...
class FloatyModeManager(context: Context) {
    ...
    fun toggleFloatyMode() {
        isFloatyModeActive = !isFloatyModeActive
        prefs.edit().putBoolean(KEY_FLOATY_MODE, isFloatyModeActive).apply()
    }

    fun setFloatyMode(enabled: Boolean) {
        isFloatyModeActive = enabled
        prefs.edit().putBoolean(KEY_FLOATY_MODE, enabled).apply()
    }
